...
import { EMPTY_ASSET, useMarketApi } from "@/lib/hooks/useApi";
...
export default function DashboardPage(properties: DashboardProperties) {
    ...
    function backtestConfigChanged(current: BacktestConfig) {
        ...
        console.log({
            ...current,
            marketAsset: {
                symbol: current.marketAsset.symbol,
                provider: current.marketAsset.provider
            },
            startDate: current.startDate.valueOf(),
            endDate: current.endDate.valueOf(),
        })

        marketApi.getPriceHistory(current.marketAsset, current.timeFrame, current.startDate.valueOf(), current.endDate.valueOf())

        // invoke('perform_backtest', {
        //     backtest: {
        //         market_asset: {
        //             symbol: current.marketAsset.symbol,
        //             provider: current.marketAsset.provider
        //         },
        //         time_frame: current.timeFrame,
        //         start_date: current.startDate.valueOf(),
        //         end_date: current.startDate.valueOf(),
        //         primary_value: current.primaryValue.valueOf(),
        //         secundary_value: current.secundaryValue?.valueOf(),
        //         signal_type: current.signalType,
        //         invert_signal: current.invertSignal
        //     }
        // }).then(x => {
        //     console.log(current)
        // })
